...
var documentType = new { documentId = "" }.GetType();
var fileContent = File.ReadAllText(@"C:\Users\deschaseauxr\Documents\DONUM\documents.json");
var array = JsonDocument.Parse(fileContent).RootElement.GetProperty("value");
...
for(var index = 0; index < nb_documents; index++) {
	...
	dynamic document = JsonSerializer.Deserialize(rawDocument, documentType);
